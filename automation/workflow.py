...
class TwoStageInspectionController:
    """Coordinate station decisions and PLC requests for one product."""
    ...
    def start_new_part(self) -> PartRecord:
        """Reset workflow state for the next incoming product."""
    ...
    def inspect_station_1(self, image: np.ndarray, source_name: str) -> StationRecord:
        """Inspect top side and either reject or release toward the flipper."""
        record = self._inspect("Station 1", image, source_name)
        self.current_part.station_1 = record
        ...
        if record.decision is StationDecision.FAIL:
            ...
            self.current_part.station_2.decision = StationDecision.SKIPPED
            self.current_part.final_disposition = FinalDisposition.REJECTED_AT_STATION_1
            self.current_part.flipper_ready = False
            self.counters.station_1_rejected += 1
        else:
            self.plc.release_to_flipper()
            self.current_part.flipper_ready = True
            self.counters.station_1_passed += 1
        return record

    def inspect_station_2(self, image: np.ndarray, source_name: str) -> StationRecord:
        """Inspect flipped side only after station one has passed."""
        if self.current_part.station_1.decision is not StationDecision.PASS:
            raise RuntimeError("Station 2 inspection requires a Station 1 PASS.")

        record = self._inspect("Station 2", image, source_name)
        self.current_part.station_2 = record
        self.counters.station_2_received += 1
        if record.decision is StationDecision.FAIL:
            record.reject_requested = True
            self.plc.reject_part("Station 2")
            self.current_part.final_disposition = FinalDisposition.REJECTED_AT_STATION_2
