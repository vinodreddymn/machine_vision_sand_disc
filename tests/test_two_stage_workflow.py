...
def test_station_1_failure_rejects_and_skips_station_2() -> None:
    ...
    assert controller.current_part.station_2.decision is StationDecision.SKIPPED
    ...
    assert plc.read_status().station_1_reject_actuator is DeviceState.ACTIVE
...
def test_station_2_failure_rejects_after_station_1_pass() -> None:
    ...
    assert record.decision is StationDecision.FAIL
    assert record.reject_requested is True
    assert controller.current_part.final_disposition is FinalDisposition.REJECTED_AT_STATION_2
    assert plc.reject_requests == ["Station 2"]
    assert controller.counters.station_2_received == 1
    assert controller.counters.station_2_rejected == 1
    assert plc.read_status().station_2_reject_actuator is DeviceState.ACTIVE


def test_station_2_cannot_run_before_station_1_passes() -> None:
    controller = TwoStageInspectionController()

    try:
        controller.inspect_station_2(build_synthetic_disk(), "bottom.png")
    except RuntimeError as error:
        assert "Station 1 PASS" in str(error)
    else:
        raise AssertionError("Station 2 should require a Station 1 pass.")
