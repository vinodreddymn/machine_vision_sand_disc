...
export function CalibrationPage() {
  ...
  const [captureResult, setCaptureResult] =
    useState<CalibrationCaptureResult | null>(null);
  const [refOd, setRefOd] = useState('100.00');
  const [refHole, setRefHole] = useState('20.00');
  const [cameraId] = useState('CAM01');
  const [saveResult, setSaveResult] = useState<CalibrationSaveResult | null>(
    null,
  );
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await saveCalibration(
        cameraId,
        captureResult.outer_diameter_px,
        od,
        hole,
      );
      setSaveResult(res);
  ...
  const handleValidate = async () => {
    ...
    setValResult(null);
    try {
      const res = await validateCalibration(
        cameraId,
        parseFloat(valRefOd),
        parseFloat(valTolerance),
      );
      setValResult(res);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
  ...
  return (
    <div className="calibration-page">
      {/* ── Status Banner ── */}
      <div
        className={`cal-status-banner ${status?.calibrated ? 'calibrated' : 'uncalibrated'}`}
      >
        ...
        <div className="cal-status-text">
          ...
          {!status?.calibrated && (
            <span>
              Place a certified reference disc under the camera and run the
        ...
        <div className="cal-status-actions">
          ...
          <button className="button" onClick={() => { void reload(); }} title="Refresh Status">
            <RefreshCcw size={14} />
          </button>
        </div>
      </div>
      ...
      {showValidate && (
        <div className="cal-panel">
          ...
          {valResult && (
            <div
              className={`cal-validation-result ${valResult.passed ? 'pass' : 'fail'}`}
            >
      ...
      <div className="cal-main-grid">
        ...
        <div className="cal-wizard-panel">
          ...
          <div className="cal-stepper">
            {STEPS.map((s, idx) => (
              <div key={s.id} className="cal-step-wrapper">
                <div
                  className={`cal-step ${step === s.id ? 'active' : step > s.id ? 'done' : ''}`}
                >
                ...
                {idx < STEPS.length - 1 && (
                  <ChevronRight
                    size={16}
                    className={`cal-step-arrow ${step > s.id ? 'done' : ''}`}
                  />
                )}
              </div>
            ))}
          ...
          {step === 1 && (
            <div className="cal-step-content">
              <h3 className="cal-step-heading">
                <Camera size={18} /> Step 1: Capture Calibration Image
              </h3>
              <p className="cal-step-desc">
                Place a certified reference disc under the camera. Then capture
                the live frame or upload a pre-recorded image.
              </p>
              <div className="cal-ref-example">
                <div className="cal-ref-badge">
                  <Target size={14} /> Reference Disc Example
                </div>
                <div className="cal-ref-specs">
                  <span>
                    Outer Diameter: <b>100.00 mm</b>
                  </span>
                  <span>
                    Center Hole: <b>20.00 mm</b>
                  </span>
                </div>
              </div>
              <div className="cal-capture-actions">
                <button
                  className="button good cal-big-btn"
                  onClick={() => { void handleCaptureLive(); }}
                  disabled={busy}
                >
                  <Camera size={18} />
          ...
          {step === 2 && captureResult && (
            <div className="cal-step-content">
              <h3 className="cal-step-heading">
                <ZoomIn size={18} /> Step 2: Circle Detection Results
              </h3>
              <p className="cal-step-desc">
                System detected the following pixel dimensions. Verify the
                overlay looks correct.
              </p>
              ...
              <img
                src={captureResult.overlay_image}
                alt="Detection Overlay"
                className="cal-overlay-img"
              />
              <div className="cal-step-actions">
                <button className="button" onClick={resetWizard}>
                  ← Re-capture
                </button>
                <button
                  className="button good"
                  onClick={handleProceedToDimensions}
                >
          ...
          {step === 3 && captureResult && (
            <div className="cal-step-content">
              <h3 className="cal-step-heading">
                <Ruler size={18} /> Step 3: Enter Actual Dimensions
              </h3>
              <p className="cal-step-desc">
              ...
              <div className="cal-scale-preview">
                ...
                <strong className="cal-scale-value">
                  {captureResult.outer_diameter_px > 0
                    ? (
                        parseFloat(refOd) / captureResult.outer_diameter_px
                      ).toFixed(6)
                    : '--'}{' '}
                  mm/pixel
                </strong>
                <span className="cal-scale-formula">
                  = {refOd} mm ÷ {captureResult.outer_diameter_px.toFixed(1)}{' '}
                  px
                </span>
              </div>
              <div className="cal-step-actions">
          ...
          {step === 4 && saveResult && (
            <div className="cal-step-content cal-success-screen">
              ...
              <p>
                Camera <b>{cameraId}</b> is now calibrated.
              </p>
        ...
        <div className="cal-history-panel">
          <div className="cal-panel-header">
            ...
            <button className="button" onClick={() => { void reload(); }} title="Refresh">
              <RefreshCcw size={14} />
            </button>
          </div>
          {history.length === 0 ? (
              ...
              <table className="cal-history-table">
                ...
                <tbody>
                  {history.map((rec) => (
                    <tr
                      key={rec.id}
                      className={rec.active ? 'cal-row-active' : ''}
                    >
                      <td>{rec.id}</td>
                      <td>
                        {new Date(rec.calibration_date).toLocaleString()}
                      </td>
                      <td className="cal-mono">
                        {rec.mm_per_pixel.toFixed(6)}
                      </td>
